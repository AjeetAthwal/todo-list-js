...
class SortFormLoader {
  ...
  _updateSort(e) {
    if (e.target.id === "Project-view")
      this.myProjects.updateSort(e.target.value);
    else if (e.target.id === "Task-view")
      this.myProjects.updateEachProjectSort(e.target.value);
    ...
    if (project !== "") {
      if (key === "dueDate")
        input.value =
          project[key] === ""
            ? ""
            : parse(project[key], "P", new Date())
                .toISOString()
                .substring(0, 10);
      else input.value = project[key];
    }
    if (key === "dueDate")
      input.min = randomProject.minDueDate.toISOString().substring(0, 10);
...
class AddProjectMethods extends AddItemMethods {
  ...
  _submitEditForm(e) {
    ...
    for (let index = 0; index < formElements.length; index++) {
      if (formElements[index].id === "title")
        newTitle = formElements[index].value;
      if (formElements[index].id === "dueDate")
        newDueDate =
          formElements[index].value === ""
            ? ""
            : new Date(formElements[index].value);
      if (formElements[index].id === "priority")
        newPriority = parseInt(formElements[index].value);
    }

    if (projectId !== "")
      this.myProjects.updateProjectInfo(
        projectId,
        newTitle,
        "BLANK",
        newPriority,
...
class AddTaskMethods extends AddItemMethods {
  ...
  _createForm(form, todo) {
    ...
    while (formParent.childElementCount > 1)
  ...
  _submitEditForm(e) {
    ...
    if (!Number.isNaN(toDoId))
      this.myProjects.updateToDoInfo(
        projectId,
        toDoId,
        newTitle,
        "BLANK",
      ...
      this.myProjects.addToDoToProject(
        projectId,
        newTitle,
        "",
        newPriority,
...
class ProjectCardTasksLoader {
  ...
  addToDosDiv(project, projectDiv) {
    ...
    project
      .getList()
      .forEach((todo) =>
        this.projectCardTaskLoader.addToDoDiv(todo, projectTodosDiv)
      );
...
class CardsLoader {
  ...
  buildCards(mainDiv) {
    ...
    this.myProjects
      .getList()
      .forEach((project) =>
        this.projectCardsLoader.buildProjectCard(project, projectsDiv)
      );
...
export {
  NewCardLoader,
  NewTaskLoader,
  ProjectCardTaskLoader,
  ProjectCardTasksLoader,
  ProjectCardExpandLoader,
  ProjectCardsLoader,
  CardsLoader,
  SortFormLoader,
  TasksPageLoader,
