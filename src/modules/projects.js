...
class Sorter {
  ...
  _compareDueDate(first, second) {
    ...
    return compareAsc(
      parse(firstDueDate, "P", new Date()),
      parse(secondDueDate, "P", new Date())
    );
  }
...
class ToDo {
  ...
  set dueDate(dateTime) {
    ...
    if (!this._isValidDate(dateTime))
      throw new Error("Please Enter a valid date");
    if (dateTime < new Date(2020, 0, 1))
      throw new Error("Please enter a date from 2020 onwards");
  ...
  set projectId(projectId) {
    ...
    if (typeof projectId !== "number")
      throw new Error("Project ID must be a non-negative Integer");
  ...
  set isComplete(newComplete) {
    if (typeof newComplete !== "boolean")
      throw new Error("ToDo isComplete must be a boolean");
  ...
  set priority(newPriority) {
    ...
    if (typeof newPriority !== "number")
      throw new Error(
        `ToDo priority must be an integer between ${this._minPriority} and ${this._maxPriority}`
      );
    if (!Number.isInteger(newPriority))
      throw new Error(
        `ToDo priority must be an integer between ${this._minPriority} and ${this._maxPriority}`
      );
    if (newPriority > this._maxPriority || newPriority < this._minPriority)
      throw new Error(
        `ToDo priority must be an integer between ${this._minPriority} and ${this._maxPriority}`
      );
...
class Project extends Sorter {
  constructor(
    ...
    if (project === "")
      this._initFromScratch(id, title, description, priority, dueDate);
...
class Projects extends Sorter {
  ...
  updateSettings() {
    ...
    if (this._toDosListener !== "" && this._toDosListener !== undefined)
      this._toDosListener.updateSettings();
  ...
  _updateToDoList() {
    ...
    for (let projectIndex = 0; projectIndex < this._list.length; projectIndex++)
      for (
        let toDoIndex = 0;
        toDoIndex < this._list[projectIndex]._list.length;
        toDoIndex++
      )
  ...
  _getProjectIndex(projectId) {
    const projectIndex = this._list.findIndex(
  ...
  _addToDoToProject(id, projectId, title, description, priority, dueDate) {
    ...
    const projectIndex = this._getProjectIndex(projectId);
    this._list[projectIndex]._addToDo(
      id,
      title,
      description,
      priority,
